module.exports = {
    ...
    execute(message, args) {
        ...
        if (!selectedRole)
            return message.reply("couldn't find the selected role");
        if (!selectedUsers.size)
            return message.reply("You need to include a user");
        ...
        selectedUsers.forEach((user) => {
            ...
            userRole.add(selectedRole).catch((err) => {
                messageChannel.send(
                    `Failed at adding ${user} to ${selectedRole}`
                ) && console.error(err);
            });
        });
    },
